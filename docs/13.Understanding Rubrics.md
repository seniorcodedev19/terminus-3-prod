...
Start: Every criterion line must begin with the word "Agent".
End: Every criterion line must end in a comma followed by the score (e.g., , +3).
Values: You must use +/- 1, 2, 3, or 5.
Signed positives: Positive scores must include an explicit leading + (e.g., +3, not 3). Unsigned positive scores will be sent back for revision.
