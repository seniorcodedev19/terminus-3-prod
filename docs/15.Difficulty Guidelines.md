...
Difficulty is measured once. After your task passes the quality panel, the platform runs the full 8-run measurement (4 trials per model, across both models), and that tier is final — nothing runs after acceptance, so the tier your reviewer sees is the one your task keeps.

At least one of the 8 runs must fail. If every run passes, the task is trivial — it produces no signal about agent capability — and it cannot proceed. Fix that by making the task harder, not by narrowing a threshold.
...
-k 4 mirrors the platform's measurement — there's no separate, shorter iteration check anymore, so your local run count matches the final one. It's still an estimate: seeds and days move results, so there's no guarantee your local read matches the platform's exactly.

Set difficulty in task.toml to your best estimate and move on. The platform's measurement after the quality panel is what gets recorded — a mismatch isn't a defect and reviewers won't spend time on it.
